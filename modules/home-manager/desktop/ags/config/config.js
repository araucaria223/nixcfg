...
function BatteryLabel() {
    ...
    //     console.log(icon)
    //     
    //     const charging = battery.bind("charging").as(ch => ch ? "charging-" : "")
    //     return `battery-${icons[icon]}-${charging}symbolic`
    ...
    const icon = battery.bind("icon-name").as(i => i)
...
function Backlight() {
    const slider = Widget.Slider({
        hexpand: true,
        draw_value: false,
        on_change: self => brightness.screen_value = self.value <= 0.01 ? 0.01 : self.value,
    ...
    function getIcon() {
        const icon = [20, 0].find(
            threshold => threshold <= brightness.screen_value * 100)
        return `weather-${icons[icon]}-symbolic`
    }
